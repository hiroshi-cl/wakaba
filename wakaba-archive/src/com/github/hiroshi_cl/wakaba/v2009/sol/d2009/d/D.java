package com.github.hiroshi_cl.wakaba.v2009.sol.d2009.d;
import java.io.*;
import java.math.*;
import java.util.*;
	...
	void run() {
		...
		for(;;)
		{
			...
			for(int i=0;i<m*2;i++){
				for(int j=1;j<=30;j++){
					for(int k=0;k<m*2;k++){
						if(ys[i] != xs[k])continue;
						int x=xs[k],y=ys[k];
						int c=cs[k],d=ds[k];
						if(j>1){
							if(j-1 <= c){
								vs[x][j][xs[i]].es.add(new E(vs[x][j][xs[i]],vs[y][j-1][x],(double)d/(j-1)));
								//								vs[y][j].es.add(new E(vs[y][j],vs[x][j-1],(double)d/(j-1)));
							}
						}
						if(j<30){
							if(j+1 <= c){
						...
						if(j<=c){
							vs[x][j][xs[i]].es.add(new E(vs[x][j][xs[i]],vs[y][j][x],(double)d/(j)));
							//							vs[y][j].es.add(new E(vs[y][j],vs[x][j],(double)d/(j)));
						}
					}
				}
			}
...
//				debug(e.to);
				...
				for(E ne:e.to.es){
					if(e.from.id == ne.to.id)continue;
