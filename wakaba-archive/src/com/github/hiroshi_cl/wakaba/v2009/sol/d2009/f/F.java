package com.github.hiroshi_cl.wakaba.v2009.sol.d2009.f;
// 未完成
import java.io.*;
import java.math.*;
import java.util.*;
	...
	void run() {
		...
		loop: for (;;) {
			...
			for (int i = 0; i < n; i++) {
				for (int j = 0; j < m - 1; j++) {
					// P p1 = st[i],p2 = st[(i+1)]
					if (right(st[j], st[j + 1], poly[i]) && perpendicularFoot_s(st[j], st[j+1], poly[j])!=null) right[i] = true;
					if (left(st[j], st[j + 1], poly[i]) && perpendicularFoot_s(st[j], st[j+1], poly[j])!=null) left[i] = true;
				}
			}
			debug(right);
			debug(left);
			...
			for (;;) {
				...
				for (;;) {
					...
					P p1 = list.remove(0), p2 = list.remove(0), p3 = list.removeFirst();
					...
					for (int j = 0; j < n; j++) {
						if (perpendicularFoot_s(p1, p3, poly[j])!=null) {
							if (!right(p1, p3, poly[j]) &&  right(p1,p3,p2)) {
								ps.add(poly[j]);
							} else if (!left(p1, p3, poly[j]) && left(p1,p3,p2)) {
								ps.add(poly[j]);
							}
						}
					}
