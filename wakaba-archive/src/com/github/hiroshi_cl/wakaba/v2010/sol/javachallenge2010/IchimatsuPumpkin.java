...
public class IchimatsuPumpkin implements Player {
...
//		int b = PumpkinType.MiniPumpkin.getNotMineLocations().size();
		debug(p);
		for (int d = 0; d < 4; d++) {
			if(!myMahojin(new Point(p.x+dx[d],p.y+dy[d])))return false;
		}
		debug(p);
		for (int d = 0; d < 8; d++) {
			if(!notMyMahojin(new Point(p.x+dx2[d],p.y+dy2[d])))return false;
		}
		return true;
//		return a == 1 && b == 0;
	}
