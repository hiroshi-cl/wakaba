...
//				debug(now.sum);
				if (now.sum > r) {
					now = now.rot(now.sum - r);
				}
			}
			System.out.println(now.a.x + " "+now.a.y);
		}
	}
	double l, r;
	int n;
	...
	class Stick {
		public String toString(){
			return "st : ("+a.toString()+") ("+c.toString()+") ("+b.toString()+")";
		}
		P c;
		P a, b;
		double sum;
		...
		Stick nextCond() {
			...
			P nc = null;
			for (P na : candA) {
				if(c.equals(na))continue;
				double nth = a.sub(b).angle(na.sub(c));
				if(eq(nth,0))continue;
				if (lt(nth , 0))
					nth += 2 * PI;
...
// if (intersect_sp(p1, p2, q1) || intersect_sp(p1, p2, q2) || intersect_sp(q1,
	...
	class P implements Comparable<P> {
		...
		public int compareTo(P o) {
			return !eq(x - o.x, 0) ? (int) signum(x - o.x) : (int) (y - o.y);
		}
